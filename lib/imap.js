...
* @this {Connection} 
* @param {string} the reference name   {@link http://www.faqs.org/rfcs/rfc3501.html} (section 6.3.8)
* @param {string} the mailbox name  (wildcards are possible)  {@link http://www.faqs.org/rfcs/rfc3501.html} (section 6.3.8)
* @param {key} optional the key name for storing the result
* @return {Connection}
*/
Connection.prototype.list=function(reference,name,key){
	...
	if (self._chained===true){
		self._appendChainCommand(cmd,(!key)?'LIST':key);
	}else{
		if (self.debug){
			self.debug('[connection] '+cmd);
		}
		self._send(cmd,(!key)?'LIST':key);
	}
	return self;
}
